...
function Profile() {
  ...
  useEffect(() => {
    async function fetchUser() {
      ...
      try {
        const response = await axios.get("https://localhost:5000/api/users/user", {
  ...
  const handleSave = async () => {
    ...
    if (!token) return alert("Please login");
    try {
      await axios.put("https://localhost:5000/api/users/user/update", {
        pseudo: username,
        email,
  ...
  const handleUpload = async () => {
    ...
    croppedCanvas.toBlob(async (blob) => {
      ...
      try {
        const response = await axios.post("https://localhost:5000/api/users/user/upload-photo", formData, {
