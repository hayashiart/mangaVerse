...
const [currentPage, setCurrentPage] = useState(1);
  ...
  const fetchUsers = async () => {
    try {
        ...
        console.log("API URL:", "https://localhost:5000/api/users/users");
      const response = await axios.get("https://localhost:5000/api/users/users", {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        const token = Cookies.get("session_token");
        await axios.delete(`https://localhost:5000/api/users/users/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
    ...
    <TableRow key={user.id_user}>
          ...
          {Array.from({ length: Math.ceil(users.length / USERS_PER_PAGE) }, (_, i) => (
            <button
              ...
              style={{
                margin: "0 8px",
                padding: "10px 16px",
                backgroundColor: currentPage === i + 1 ? "#518cc7" : "#2a2a2a",
                color: "white",
                border: "none",
                borderRadius: "6px",
                cursor: "pointer",
        ...
        <AddButton
          onClick={async () => {
            try {
              ...
              const token = Cookies.get("session_token");
              const response = await axios.post("https://localhost:5000/api/users/users", newUser, {
                headers: { Authorization: `Bearer ${token}` },
              });
              await fetchUsers();
      ...
      <SaveButton
        onClick={async () => {
          try {
            const token = Cookies.get("session_token");
            const originalUsers = await axios.get("https://localhost:5000/api/users/users", {
              headers: { Authorization: `Bearer ${token}` },
            }).then(res => res.data);
            ...
            for (const user of updatedUsers) {
              await axios.put(`https://localhost:5000/api/users/users/${user.id_user}`, user, {
                headers: { Authorization: `Bearer ${token}` },
              });
            }
