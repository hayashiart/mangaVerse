...
router.put("/users/:id", authenticateJWT, async (req, res) => {
  ...
  const { id } = req.params;
  const { pseudo, email, role, password } = req.body; // maintenant on accepte tout

  try {
    let query = "UPDATE Users SET pseudo = ?, email = ?, role = ? WHERE id_user = ?";
    let params = [pseudo, email, role, id];

    if (password) {
      const hashedPassword = await bcrypt.hash(password, 10);
      query = "UPDATE Users SET pseudo = ?, email = ?, role = ?, password = ? WHERE id_user = ?";
      params = [pseudo, email, role, hashedPassword, id];
    }
